...
void computeSubtask(Matrix* firstMatrix, Matrix* secondMatrix, Matrix* resultingMatrix, int startingRowIndex, int rowsComputed, int taskIndex) {
...
Matrix multiplyMatrices(Matrix* firstMatrix, Matrix* secondMatrix) {
    ...
    for (int threadIndex = 0; threadIndex < numberOfConcurrentThreads - 1; threadIndex++) {
        threads.emplace_back(computeSubtask, firstMatrix, secondMatrix, &matrix, rowIndex, rowsComputedByOneThread, threadIndex);
        rowIndex += rowsComputedByOneThread;
    }

    threads.emplace_back(computeSubtask, firstMatrix, secondMatrix,
                         &matrix, rowIndex,
    ...
    std::cout << "The number of rows computed by one thread with maximum possible concurrency is " << rowsComputedByOneThread << "\n";
    std::cout << "The number of threads used: " << threads.size() << "\n";
